...
impl Violet {
    pub const ID: &'static str = "violet_noise";
    pub const LABEL: &'static str = "Violet Noise";
    pub const DESCRIPTION: &'static str = "White noise pushed hard into the highs. Thin and sharp, the brightest of the noise colors.";
}

impl Default for Violet {
    fn default() -> Self {
        Self {
            amplitude: 0.5,
            seed: 1,
...
impl Node for Violet {
    ...
    fn process(&mut self, _input: f32, _ctx: &Context) -> f32 {
        ...
        let violet = (white - 2.0 * self.previous + self.older) * 0.25;
        self.older = self.previous;
        self.previous = white;
        violet * self.amplitude
    }
